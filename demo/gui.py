...
class CCReplicateFG (tk.Toplevel):
    def __init__ (self, parent, *args, **kwargs):
        ...
        # Source
        frame = tk.Frame(self)
        frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=1)
        canvas = CCCanvas(frame)
        label = tk.Label(frame, text="Source")
        entry = tk.Entry(frame)
        button = tk.Button(frame, text="...")
        label.grid(row=0, column=0, columnspan=2, sticky="ew")
        canvas.widget.grid(row=1, column=0, columnspan=2, sticky="nsew")
        ...
        # Target
        frame = tk.Frame(self)
        frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=1)
        canvas = CCCanvas(frame)
